...
data <- list(N = N, D = D, M = M, 
             ...
             y = y, rel_T = rel_T)
write_rds(data, str_glue("data/{model_name}.rds"))

cat("Run MCMC by Stan \n")

output_dir_lmm = str_glue("stan/draws/{model_name}")
...
write_csv(lmm_summary, str_glue("stan/summary/{model_name}.csv"))

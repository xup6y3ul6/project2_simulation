...
```{r}
model_name <- "sim_3l-lmm_ARdHdARmHm_N10D20M20Seed20250603"
...
ppc_intervals(y = data[pars_sd] |> unlist(), 
  ...
  scale_x_discrete(name = "Standard deviations", 
                   limits = c("psi_s", str_glue("psi_d[{j}]", j = 1:D), "sigma_omega_d", "sigma_omega_m", str_glue("sigma_epsilon[{k}]", k = 1:M)))
```
...
data_predict_lmm |> 
  # filter(Subject %in% selected_subj) |> 
